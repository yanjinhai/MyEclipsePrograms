...
public class SudokuSolverRevamp {
	...
	public static void main(String[] args) {
		initialize();
		boolean b = oneCell(0, problem, work);
		if (b)
			printSuccess();
	}
	...
	static boolean oneCell(int nCell, int[][] prob, int[][] newWork) {
		...
		if (prob[i][j] != 0) {
			return oneCell(nCell + 1, prob, newWork);
		}
		...
		for (int k = 0; k < 9; k++) {
			possible.remove(Integer.valueOf(newWork[i][k]));
		}
		for (int k = 0; k < 9; k++) {
			possible.remove(Integer.valueOf(newWork[k][j]));
		}
		int m = 3 * (i / 3);
		int n = 3 * (j / 3);
		for (int k = m; k < m + 3; k++) {
			for (int l = n; l < n + 3; l++) {
				possible.remove(Integer.valueOf(newWork[k][l]));
			}
		}
		...
		while (!possible.isEmpty()) {
			...
			newWork[i][j] = mytry;
			boolean b = oneCell(nCell + 1, prob, newWork);
			if (b) {
				return true;
			}
		}
